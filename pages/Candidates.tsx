...
const Candidates: React.FC = () => {
  usePageMeta(
    'For Candidates | RexJagers Executive Search',
    'Advance your career with RexJagers. We partner with exceptional leaders, offering confidential guidance and access to premier executive opportunities.'
  );
            ...
            <ul className="space-y-5 text-navy/75 dark:text-slate">
              {CANDIDATE_COMMITMENTS.map((item) => (
                <li key={item.title} className="flex items-start">
                  ...
                  <span>
                    <strong className="text-navy dark:text-white">{item.title}:</strong> {item.text}
                  </span>
                </li>
              ))}
            </ul>
